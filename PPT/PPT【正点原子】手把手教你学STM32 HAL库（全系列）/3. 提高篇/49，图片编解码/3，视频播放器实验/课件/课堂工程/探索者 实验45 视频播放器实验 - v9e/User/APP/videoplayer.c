...
void video_info_show(AVI_INFO *aviinfo)
{
    ...
    text_show_string(10, 50, lcddev.width - 10, 16, (char *)buf, 16, 0, RED);   /* 显示音频信息 */
    
    sprintf((char *)buf, "帧率:%d帧", 1000 / (aviinfo->SecPerFrame / 1000));
    text_show_string(10, 70, lcddev.width - 10, 16, (char *)buf, 16, 0, RED);   /* 显示视频信息 */
...
void video_play(void)
{
    ...
    if (res == FR_OK)
    {
        ...
        while (1)       /* 全部查询一遍 */
        {
            ...
            res = exfuns_file_type(vfileinfo->fname);   /* 通过文件名判断文件类型 */
            if ((res & 0xF0) == 0x60)           /* 取高四位,看看是不是avi文件 */
            {
                voffsettbl[curindex] = temp;    /* 记录VIDEO文件夹中avi文件名位置偏移 */
                curindex++;
            }
        }
    }
    ...
    {
        dir_sdi(&vdir, voffsettbl[curindex]);   /* 改变当前目录索引 从第一个avi开始 */
        res = f_readdir(&vdir, vfileinfo);      /* 读取目录下的一个文件 */
        if (res != FR_OK || vfileinfo->fname[0] == 0)
        {
            break;          /* 错误了/到末尾了,退出 */
        ...
        key = video_play_mjpeg(pname);  /* 播放这个avi文件 */
 ...
 * @param       pname : 文件名
 * @retval      res
 *   @arg       KEY0_PRES/KEY2_PRES , 下一个/上一个
 *   @arg       KEY1_PRES/WKUP_PRES , 快进/后退
 *   @arg       其他 , 错误
 */
uint8_t video_play_mjpeg(uint8_t *pname)
{
    ...
    while (res == 0)
    {
        ...
        {
            pbuf = framebuf;
            res = f_read(favi, pbuf, AVI_VIDEO_BUF_SIZE, &nr);  /* 开始读取avi文件数据 */
            if (res)
            {
...
//            res = mjpegdec_init((lcddev.width - g_avix.Width) / 2, 110 + (lcddev.height - 110 - g_avix.Height) / 2);    /* JPG解码初始化 */
            ...
            {
                res = mjpegdec_init((lcddev.width - g_avix.Width) / 2, (lcddev.height - g_avix.Height) / 2);              /* JPG解码初始化 */
            }
            else
            {
                res = mjpegdec_init((lcddev.width - g_avix.Width) / 2, 110 + (lcddev.height - 110 - g_avix.Height) / 2);  /* JPG解码初始化 */
            ...
            while (1)   /* 播放循环 */
            {
                ...
                else    /* 音频流 */
                {
                    ...
                    } while (i2ssavebuf == nr); /* 碰撞等待(避免同时访问一个buf,导致声音异常) */
                ...
                {
                    i2s_play_stop();    /* 关闭音频 */
                    video_seek(favi, &g_avix, framebuf);    /* 该函数实现快进快退 */
                    pbuf = framebuf;

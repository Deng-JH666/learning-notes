...
void btim_tim7_int_init(uint16_t arr, uint16_t psc)
{
    ...
    HAL_NVIC_SetPriority(BTIM_TIM7_INT_IRQn, 1, 3);         /* 抢占1，子优先级3，组1 */
    HAL_NVIC_EnableIRQ(BTIM_TIM7_INT_IRQn);                 /* 开启TIM7中断 */
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == BTIM_TIM6_INT)
    {
        ...
        HAL_NVIC_SetPriority(BTIM_TIM6_INT_IRQn, 1, 3); /* 抢占1，子优先级3，组1 */
...
{
    if (htim == (&g_tim6_handler))              /* 定时器6 */
    {
        printf("video frame:%d\r\n", frame);    /* 打印帧率 */
        frame = 0;
        LED0_TOGGLE();
    }
    if(htim == (&g_tim7_handler))               /* 定时器7 */
    {
        frameup = 1;
        LED1_TOGGLE();
    }

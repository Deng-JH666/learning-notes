...
/* sd_read_disk/sd_write_disk函数专用buf,当这两个函数的数据缓存区地址不是4字节对齐的时候,
...
SD_Error sd_init(void)
{
    ...
    {
        errorstatus = sd_initialize_cards();    /* 初始化SD卡 */
    }
    
    if (errorstatus == SD_OK)
    {
        errorstatus = sd_get_card_info(&g_sd_card_info);    /* 获取卡信息 */
    }
    
    if (errorstatus == SD_OK)
    {
        errorstatus = sd_select_deselect((uint32_t)(g_sd_card_info.RCA << 16));  /* 选中SD卡 */
    }
    
    if (errorstatus == SD_OK)
    {
        errorstatus = sdio_wide_bus_operation(1);  /* 4位宽度,如果是MMC卡,则不能用4位模式 */
    }
    ...
    if ((errorstatus == SD_OK) || (MULTIMEDIA_CARD == CardType))
    {
        ...
        /* 设置时钟频率,SDIO时钟计算公式: SDIO_CK 时钟 = SDIOCLK / [2 * clkdiv]; 其中, SDIOCLK固定为48Mhz */
 ...
 * @brief       SDIO 时钟设置
 * @param       clkdiv : 时钟分频系数
 *   @note      CK时钟 =  SDIOCLK / [2 * clkdiv]; (SDIOCLK时钟固定为48Mhz)
 * @retval      无
 */
static void sdio_clock_set(uint16_t clkdiv)
{
 ...
 * @retval      无
 */
static void sdio_send_cmd(uint8_t cmdindex, uint8_t waitrsp, uint32_t arg)
{
    uint32_t tmpreg = 0;
 ...
 * @retval      无
 */
static void sdio_send_data_cfg(uint32_t datatimeout, uint32_t datalen, uint8_t blksize, uint8_t dir)
{
    uint32_t tmpreg;
 ...
 */
static SD_Error sd_power_on(void)
{
    uint8_t i = 0;
    uint32_t tempreg = 0;
    SD_Error errorstatus = SD_OK;
    ...
    tempreg |= 0 << 10;     /* BYPASS=0,关闭旁路，CK根据分频设置输出 */
    tempreg |= 0 << 11;     /* WIDBUS[1:0]=0,1位数据宽度 */
    tempreg |= 0 << 13;     /* NEGEDGE=0,SDIOCK下降沿更改命令和数据 */
    ...
    for (i = 0; i < 74; i++)
    {
        sdio_send_cmd(SD_CMD_GO_IDLE_STATE, 0, 0);     /* 发送CMD0进入IDLE STATE模式命令. */
        errorstatus = sdio_cmd_error();
     ...
     * 返回响应7
     */
    sdio_send_cmd(SD_SDIO_SEND_IF_COND, 1, SD_CHECK_PATTERN); 
    
    errorstatus = sdio_cmd_resp7_error();       /* 等待R7响应 */
    ...
    errorstatus = sdio_cmd_resp1_error(SD_CMD_APP_CMD);/* 等待R1响应 */
    ...
    if (errorstatus == SD_OK)   /* SD2.0/SD 1.1,否则为MMC卡 */
    {
        ...
        while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
        {
            ...
            errorstatus = sdio_cmd_resp3_error();  /* 等待R3响应 */
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sd_initialize_cards(void)
{
    SD_Error errorstatus = SD_OK;
    uint16_t rca = 0x01;
    ...
    if (SECURE_DIGITAL_IO_CARD != CardType)         /* 非SECURE_DIGITAL_IO_CARD */
    {
        ...
        errorstatus = sdio_cmd_resp2_error();       /* 等待R2响应 */
    ...
    if ((STD_CAPACITY_SD_CARD_V1_1 == CardType) || (STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SECURE_DIGITAL_IO_COMBO_CARD == CardType) || (HIGH_CAPACITY_SD_CARD == CardType))
    {
        ...
        errorstatus = sdio_cmd_resp6_error(SD_CMD_SET_REL_ADDR, &rca); /* 等待R6响应 */
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sd_get_card_info(SD_CardInfo *cardinfo)
{
    SD_Error errorstatus = SD_OK;
    uint8_t tmp = 0;
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_wide_bus_operation(uint32_t wmode)
{
    SD_Error errorstatus = SD_OK;
    uint16_t clkcr = 0;
    ...
    else if ((STD_CAPACITY_SD_CARD_V1_1 == CardType) || (STD_CAPACITY_SD_CARD_V2_0 == CardType) || (HIGH_CAPACITY_SD_CARD == CardType))
    {
        ...
        {
            errorstatus = sdio_wide_bus_enable(wmode);
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sd_select_deselect(uint32_t addr)
{
    sdio_send_cmd(SD_CMD_SEL_DESEL_CARD, 1, addr);      /* 发送CMD7,选择卡,短响应 */
    return sdio_cmd_resp1_error(SD_CMD_SEL_DESEL_CARD);
}
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_read_blocks(uint8_t *pbuf, long long addr, uint16_t blksize, uint32_t nblks)
{
    SD_Error errorstatus = SD_OK;
    uint32_t count = 0;
    ...
    errorstatus = sdio_cmd_resp1_error(SD_CMD_SET_BLOCKLEN);    /* 等待R1响应 */
    ...
    if (nblks > 1)  /* 多块读 */
    {
        ...
        errorstatus = sdio_cmd_resp1_error(SD_CMD_READ_MULT_BLOCK); /* 等待R1响应 */
    ...
    else    /* 单块读 */
    {
        ...
        errorstatus = sdio_cmd_resp1_error(SD_CMD_READ_SINGLE_BLOCK);  /* 等待R1响应 */
    ...
    while (!(SDIO->STA & ((1 << 5) | (1 << 1) | (1 << 3) | (1 << 8))))    /* 无上溢/CRC/超时/完成(标志) */
    {
        if (SDIO->STA & (1 << 15))                  /* 接收区半满,表示至少存了8个字 */
        {
            ...
            tempbuff += 8;
            timeout = SDIO_DATATIMEOUT;            /* 读数据溢出时间 */
        }
        else    /* 处理超时 */
        {
            if (timeout == 0)
    ...
    else if (SDIO->STA & (1 << 1))  /* 数据块CRC错误 */
    {
        ...
        if (nblks > 1)              /* 针对可能出现的CRC错误,如果是多块读取,必须发送结束传输命令! */
        {
            ...
            errorstatus = sdio_cmd_resp1_error(SD_CMD_STOP_TRANSMISSION);  /* 等待R1响应 */
        }
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_write_blocks(uint8_t *pbuf, long long addr, uint16_t blksize, uint32_t nblks)
{
    SD_Error errorstatus = SD_OK;
    uint8_t  cardstate = 0;
    ...
    if (nblks > 1)  /* 多块写 */
    {
        ...
        if ((STD_CAPACITY_SD_CARD_V1_1 == CardType) || (STD_CAPACITY_SD_CARD_V2_0 == CardType) || (HIGH_CAPACITY_SD_CARD == CardType))
        {
            ...
            errorstatus = sdio_cmd_resp1_error(SD_CMD_SET_BLOCK_COUNT); /* 等待R1响应 */

            if (errorstatus != SD_OK) return errorstatus;
        }

        sdio_send_cmd(SD_CMD_WRITE_MULT_BLOCK, 1, addr);            /* 发送CMD25,多块写指令,短响应 */
        errorstatus = sdio_cmd_resp1_error(SD_CMD_WRITE_MULT_BLOCK);    /* 等待R1响应 */
    }
    else    /* 单块写 */
    {
        sdio_send_cmd(SD_CMD_SEND_STATUS, 1, (uint32_t)RCA << 16);  /* 发送CMD13,查询卡的状态,短响应 */
        errorstatus = sdio_cmd_resp1_error(SD_CMD_SEND_STATUS);     /* 等待R1响应 */
        ...
        errorstatus = sdio_cmd_resp1_error(SD_CMD_WRITE_SINGLE_BLOCK); /* 等待R1响应 */
    }
    ...
    while (!(SDIO->STA & ((1 << 4) | (1 << 1) | (1 << 8) | (1 << 3))))    /* 下溢/CRC/数据结束/超时 */
    {
        if (SDIO->STA & (1 << 14))                              /* 发送区半空,表示至少存了8字(32字节) */
        {
            ...
            timeout = SDIO_DATATIMEOUT;    /* 写数据溢出时间 */
        }
        else
        {
            if (timeout == 0)
    ...
    SDIO->ICR = 0X1FE00FFF;           /* 清除所有标记 */
    errorstatus = sdio_is_card_programming(&cardstate);
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_cmd_error(void)
{
    SD_Error errorstatus = SD_OK;
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_cmd_resp1_error(uint8_t cmd)
{
    uint32_t status;
    uint32_t timeout = SDIO_CMD1TIMEOUT;
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_cmd_resp2_error(void)
{
    SD_Error errorstatus = SD_OK;
    uint32_t status;
    uint32_t timeout = SDIO_CMD1TIMEOUT;
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_cmd_resp3_error(void)
{
    uint32_t status;
    uint32_t timeout = SDIO_CMD1TIMEOUT;
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_cmd_resp6_error(uint8_t cmd, uint16_t *prca)
{
    SD_Error errorstatus = SD_OK;
    uint32_t status;
    uint32_t rspr1;
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_cmd_resp7_error(void)
{
    SD_Error errorstatus = SD_OK;
    uint32_t status;
    uint32_t timeout = SDIO_CMD0TIMEOUT;
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_wide_bus_enable(uint8_t enx)
{
    SD_Error errorstatus = SD_OK;
    uint32_t scr[2] = {0, 0};
    uint8_t arg = 0X00;
    ...
    errorstatus = sdio_find_scr(RCA, scr);        /* 得到SCR寄存器数据 */
    ...
    if ((scr[1]&SD_WIDE_BUS_SUPPORT) != SD_ALLZERO) /* 支持宽总线 */
    {
        ...
        errorstatus = sdio_cmd_resp1_error(SD_CMD_APP_CMD);

        if (errorstatus != SD_OK) return errorstatus;

        sdio_send_cmd(SD_CMD_APP_SD_SET_BUSWIDTH, 1, arg);     /* 发送ACMD6,短响应,参数:10,4位;00,1位. */
        errorstatus = sdio_cmd_resp1_error(SD_CMD_APP_SD_SET_BUSWIDTH);
        return errorstatus;
    }
    else
    {
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_is_card_programming(uint8_t *pstatus)
{
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_send_status(uint32_t *pstatus)
{
    SD_Error errorstatus = SD_OK;
    ...
    errorstatus = sdio_cmd_resp1_error(SD_CMD_SEND_STATUS); /* 查询响应状态 */
 ...
 */
SDCardState sdio_get_status(void)
{
    uint32_t resp1 = 0;

    if (sdio_send_status(&resp1) != SD_OK)
    {
 ...
 * @retval      0, 正常;  其他, 错误代码(详见SD_Error定义);
 */
static SD_Error sdio_find_scr(uint16_t rca, uint32_t *pscr)
{
    SD_Error errorstatus = SD_OK;
    uint32_t tempscr[2] = {0, 0};
    
    sdio_send_cmd(SD_CMD_SET_BLOCKLEN, 1, 8);      /* 发送CMD16,短响应,设置Block Size为8字节 */
    errorstatus = sdio_cmd_resp1_error(SD_CMD_SET_BLOCKLEN);
    ...
    errorstatus = sdio_cmd_resp1_error(SD_CMD_SD_APP_SEND_SCR);
...
uint8_t sd_read_disk(uint8_t *pbuf, uint32_t saddr, uint32_t cnt)
{
    ...
    if ((uint32_t)pbuf % 4 != 0)    /* 数据缓存区首地址必须是4的倍数 */
    {
        for (n = 0; n < cnt; n++)
        {
            sta = sdio_read_blocks(g_sd_data_buffer, lsaddr + 512 * n, 512, 1);    /* 单个sector的读操作 */
    ...
    {
        sta = sdio_read_blocks(pbuf, lsaddr, 512, cnt);    /* 读取单个/多个sector */
    }
...
uint8_t sd_write_disk(uint8_t *pbuf, uint32_t saddr, uint32_t cnt)
{
    ...
    if ((uint32_t)pbuf % 4 != 0)    /* 数据缓存区首地址必须是4的倍数 */
    {
        for (n = 0; n < cnt; n++)
        {
            ...
            sta = sdio_write_blocks(g_sd_data_buffer, lsaddr + 512 * n, 512, 1); /* 单个sector的写操作 */
    ...
    {
        sta = sdio_write_blocks(pbuf, lsaddr, 512, cnt);   /* 写单个/多个sector */
    }
